...
///这个只需要在内部确定 因为同一个APP样式是统一的
///连击数字视图是否在飘屏视图上 并随着数字位数的增加而向左压缩
@property (nonatomic,assign) BOOL isCountViewInView;
...
- (void)setupContentContraints{
    ...
    if (self.isCountViewInView) {
        ...
        [self.giftIV mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(self.self.numberView.mas_left).offset(-10.0);
        ...
        [self.nameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(self.iconIV.mas_right).offset(6.0);
            make.width.mas_lessThanOrEqualTo(100.0);
        }];
        
        [self.sendLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.nameLabel.mas_bottom).offset(5.0);
            make.left.equalTo(self.nameLabel);
            make.right.equalTo(self.giftIV.mas_left).offset(-15.0);
        }];
      
        
    }else{
        //连击数量在视图右边单独显示不会随着数字位数的增加而向左压缩礼物图片
        [self.nameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.mas_top).offset(6.0);
            make.left.equalTo(self.iconIV.mas_right).offset(6.0);
            make.width.mas_lessThanOrEqualTo(100.0);
        ...
        [self.numberView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.mas_right).offset(kGiftNumberWidth);
            make.centerY.height.equalTo(self);
            make.width.mas_equalTo(kGiftNumberWidth);
        }];
    }
